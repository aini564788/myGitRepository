...
<link rel="stylesheet" href="${pageContext.request.contextPath}/static/js/jquery-ztree/css/demo.css">
<link rel="stylesheet" href="${pageContext.request.contextPath}/static/js/jquery-ztree/css/zTreeStyle/zTreeStyle.css">
<script src="${pageContext.request.contextPath}/static/js/jquery-ztree/js/jquery.ztree.all-3.5.min.js"></script>
...
<script type="text/javascript">
    ...
    layui.use(['table','form','element'],function () {
        ...
        tableIns=table.render({
            elem: '#demo'
            ,height: 312
            ,url: '${pageContext.request.contextPath}/role/list/query' //数据接口
            ,title: '用户表'
            ,page: true //开启分页
        ...
        table.on('toolbar(test)', function(obj){
            var checkStatus = table.checkStatus(obj.config.id)
                ,data = checkStatus.data; //获取选中的数据  通过.属性  获取相应的值
           // alert(checkStatus+"--------->");
            switch(obj.event){
                case 'add':
                   // layer.msg('添加');
                ...
                case 'update':
                    if(data.length === 0){
                        ...
                        $.ajax({
                            url:'${pageContext.request.contextPath}/role/modify/query',
                        ...
                        layer.open({
                            ...
                            yes: function (index, layero) {
                                //alert($("#fm1").serialize());
                                $.ajax({
                                    url: "${pageContext.request.contextPath}/role/modify/do",
                                    //表单序列化
                                    data: $("#fm1").serialize(),
                                    success: function (obj) {
                                        if (obj.code==0) {
                                            layer.msg("编辑角色成功", {icon: 6});
                                            tableIns.reload();

                                        } else {
                                            layer.msg("编辑角色失败", {icon: 5});
                                        }
                                    }
                                })
    ...
    function  deleBatch(ids) {
            $.ajax({
            url:'${pageContext.request.contextPath}/role/delete/batch',
            dataType:'json',
    ...
    function  delById(id,obj) {
        $.ajax({
            url:'${pageContext.request.contextPath}/role/delete/single/'+id,
            type:'POST',
            data:{_method:"DELETE"},
            cache:false,
            success:function (res) {
    ...
    function  showTree(obj) {
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/role/ztree/query',
    ...
    function editPermission(index,data) {
         ...
         */
             ...
             */
            $.ajax({
                url:'${pageContext.request.contextPath}/role/ztree/update',
...
<div style="display: none;margin-top: 10px;width: 480px" id="dvlay">
    <form class="layui-form"  id="fm1"  action="${pageContext.request.contextPath}/role/modify/do" method="post">
        <div class="layui-form-item">
